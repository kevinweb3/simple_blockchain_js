...
export class Block extends EventEmitter {
  constructor(data, consensus) {
    ...
    this.generator_publickey_ = data
  ...
  get_publickey() {
    return this.generator_publickey_;
  }
  ...
  get_consensus_data() {
    return this.consensus_data_;
  }

  set_consensus_data(data) {
    this.consensus_data_ = data;
  }
  ...
  calc_merkle_hash() {
    const hashes = [];
    for (const i = 0; i < this.transactions_.length; ++i) {
      hashes.push(Crypto.calc_hash(this.transactions_.toString("utf-8")));
    }
    while (hashes.length > 1) {
      const tmp = [];
      for (const i = 0; i < hashes.length / 2; ++i) {
        const data = hashes[i * 2] + hashes[i * 2 + 1];

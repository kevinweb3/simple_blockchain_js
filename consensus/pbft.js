...
const PBFT_N = slot.delegates;
const PBFT_F = Math.floor((PBFT_N - 1) / 3);
...
export class Pbft {
  ...
  commit(hash) {
    const block = this.pending_block_[hash];
    block.emit("consensus completed", block.toObject());
    this.clear_state_();
  }

  processMessage(msg) {
    const key = msg.data.hash + ":" + msg.data.height + ":" + msg.data.signer;
    switch (msg.type) {
      ...
      case MessageType.Commit:
        ...
        const commit = this.commit_infos_[msg.data.hash];
        if (commit) {
